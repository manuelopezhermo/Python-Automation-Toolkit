...
class Table:
    ...
    def buscar_cabeceras(self, cabeceras: list, sheet_name: str):
            ...
            dict_coordenadas_cabeceras = defaultdict(list)
            for cabecera in cabeceras:
                cabecera_buena = normalizar(cabecera)
                for row in ws.iter_rows():
                    for cell in row:
                        cell_value = normalizar(cell.value)
                        if cell_value == cabecera_buena:
                            coordenada_merge = self.workbook.merge_manager.get_merge_coordinate_value(cell, sheet_name)
                            if coordenada_merge == None:
                                dict_coordenadas_cabeceras[cabecera].append(cell.coordinate)
                            else:
                                dict_coordenadas_cabeceras[cabecera].append(coordenada_merge)
            print(dict_coordenadas_cabeceras)
            input("IO")
            return dict_coordenadas_cabeceras
    ...
    def recoger_tabla_con_filas(self, cabeceras: list, numero_filas: int, sheet_name: str):
        ...
        cabeceras_superiores = self.buscar_cabeceras(cabeceras, sheet_name)
        lista_celdas_tabla = []
        for nombre_cabecera, coordenada in cabeceras_superiores.items():
            fila, columna = trasformar_coordenadas_a_numeros(coordenada)
            lista_celdas_tabla.append(self.workbook.wb[sheet_name].cell(fila, columna))
        lista_celdas_tabla = self.obtener_tabla_numero_filas(lista_celdas_tabla, numero_filas, sheet_name)
        return lista_celdas_tabla
